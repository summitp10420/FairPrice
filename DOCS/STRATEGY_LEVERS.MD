...
Example:
- Remote Strategy Resolution lever (Sprint 13): Railway engine decides which strategy to send; app executes. See `DOCS/STRATEGY_ENGINE_TERMINOLOGY.MD`.
...
3.0 Remote Strategy Resolution Lever (Sprint 13/14)
What it controls:
- Where strategy decisions are made (Railway backend).
- How the app obtains a strategy (RailwayStrategyClient + LocalStrategyFallback).
- Which profile is selected (tactic-driven vs epsilon-greedy exploration).

Approaches:
- Railway strategy engine returns StrategyResult from (domain, detected_tactics, anonymous_bucket).
- Engine uses in-memory cache (strategy_profiles, tactic_registry); 60s background refresh. Zero DB calls on critical path.
- Tactic-driven (exploit): merge required_countermeasures from tactic_registry → select lowest-tier qualifying profile.
- Epsilon-greedy (75/25): bucket 0–74 = exploit; bucket 75–99 = explore (deterministic tier = bucket % 4).
- App receives, loads, executes. Admin override (EngineOverride) can force a specific profile when isAdmin.

Actions:
- Call Railway POST /api/v1/strategy.
- On timeout/unreachable: use LocalStrategyFallback.
- Execute returned strategy (profile code, flat levers, proxy config when available).
- Admin: force clean_baseline, shield_basic, amnesia_standard, or stealth_max via override.

Policy knobs:
- Endpoint URL.
- Timeout duration.
- Fallback strategy (cold-boot: amnesia_standard).
- Exploration rate (currently 25%).

Key telemetry:
- Railway call success/failure.
- Strategy source (railway vs fallback).
- engineSelectionPolicy, strategy_profile, selection_mode (exploit/explore) from StrategyResult.

3.1 VPN Config Pool Management Lever [DEPRECATED for clear-net phase]
What it controls:
...
3.2 VPN Rotation Policy Lever [DEPRECATED for clear-net phase]
What it controls:
...
3.3 Baseline Identity Lever [DEPRECATED for clear-net phase]
What it controls:
...
- Classify WAF vendors (`vendor_perimeterx`, `vendor_datadome`, `vendor_akamai`, `vendor_cloudflare`).
- Detect block-page signatures and emit `debugExtractionPath = "waf_block"` with `block_*` tactics.
...
- Sniffer vs clean-control vs spoof extraction success.
- Tactic detection frequency.
- Negative-feedback coverage (block events captured without timeout loss).
...
- Sniffer extraction lift from canonicalized URLs.
...
- Use sampled shadow clean-control (20% beta) for counterfactual telemetry without adding a second spoof chain.
...
Key telemetry:
- Degraded run rate.
- Primary failure stage (sniffer, clean_control, connect, spoof, logging).
- Sniffer `waf_block` fallback rate and clean-control recovery rate.
...
- Persist pass provenance (`tactic_source_pass`, clean-control execution mode, shadow sampled flag).
...
- Counterfactual coverage from sampled clean-control shadow runs.

URL bootstrap guardrail:
- Tests and runtime must follow one `fp_engine` hash contract:
  - append as `#fp_engine=<token>` when no fragment exists
  - append/replace as `&fp_engine=<token>` when fragment exists
  - never expect `#...#fp_engine` double-hash formatting.
